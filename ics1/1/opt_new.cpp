...
{   SumQuantity=0;
    ...
    for(int i=lengthd*withd;i<Table.num;i++){
        if(date_start[i]<=n){
            SumQuantity+=quantity_start[i];
            SumBasePrice+=price_start[i];
            double disc_price=price_start[i]*(1-discount_start[i]);
            SumDiscPrice+=disc_price;
            SumCharge+=disc_price*(1+tax_start[i]);
            AvgDiscount+=discount_start[i];
            total++;
        }
    }
    int temp=0;
